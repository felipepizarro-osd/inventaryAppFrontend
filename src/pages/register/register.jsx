...
//Registrado
const useStylesR = makeStyles(theme => ({
  root: {
    backgroundImage: `url(${backgroundHome})`,
    backgroundRepeat: 'no-repeat',
    backgroundSize: 'cover',
...
const Register=()=> {
  ...
  if(localStorage.getItem('isLogin')!==null){
    if(localStorage.getItem('registerdone')){
      return (
        <Grid container component='main' className={classesR.root}>
          <Sidebar/>
            <CssBaseline />
            <Container component={Paper} elevation={5} maxWidth='xs' className={classesR.container}>
                <div className={classesR.div}>
                    <Avatar className={classesR.avatar}>
                        <CheckCircleIcon />
                    </Avatar>
                    <Typography component='h1' variant='h5'>Register done</Typography>
                    <form className={classesR.form}>
                    <Button
                          fullWidth
                          variant='contained'
                          color='secondary'
                          className={classesR.button}
                          onClick={()=> backtoHome()}
                      >
                        Back to home.
                      </Button>
